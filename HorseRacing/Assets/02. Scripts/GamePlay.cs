...
public class GamePlay : MonoBehaviour
{
    ...
    public List<GameObject> players = new List<GameObject>(); // Hierarchy 에서 받으려고 public 씀 
    ...
    {
        for (int i = players.Count - 1; i > -1; i--) // (int i = players.Count -1; i >= 0; i++) 가능,                     for문을 거꾸로 돌리는 이유: 원래대로 하면 플레이어 1명이 안 세짐
        {
    ...
    {
        if (finishedPlayers.Count >= totalPlayers) // == 해도 되지만 대부분 >= 사용
        {
            onPlay = false; 
