...
// 값형식은 말 그대로 값을 읽고 쓰는 형식,
// 값형식의 종류 : 일반적인 데이터 타입들 ( int, float, double, enum, structure 등 ... )
// 참조형식은 주소를 참조해서 해당 주소의 값을 읽고 쓰는 형식 
// 참조형식의 종류 : 클래스, 인터페이스, 델리게이트 
// 참조형식들은 기본적으로 Serialize가 안 됨 ( 주소값을 그대로 Serialize 하는 것은 의미가 없기 때문 )
// 하지만 Serialzable 속성을 주면 
// 참조 형식을 Serialize 시도할 때 해당 주소의 힙영역에 있는 값을 읽어옴.

[System.Serializable] // 데이터 저장하기 위함  // 해당 클래스 타입의 오브젝트가 Serialize 가능하게 해주는 속성 
